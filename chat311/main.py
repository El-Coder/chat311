...
import chat311.chat
import chat311.commands as cmd
import chat311.utils
from chat311.ai_config import AIConfig
from chat311.config import Config
from chat311.json_parser import fix_and_parse_json
from chat311.logger import logger
from chat311.memory import get_memory, get_supported_memory_backends
from chat311.prompt import get_prompt
from chat311.spinner import Spinner
...
def print_assistant_thoughts(assistant_reply, ai_name=""):
    ...
    try:
        ...
        logger.typewriter_log(
            "REASONING:", Fore.YELLOW, assistant_thoughts_reasoning
        )
        ...
        logger.typewriter_log(
            "CRITICISM:", Fore.YELLOW, assistant_thoughts_criticism
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--speak", action="store_true", help="Enable Speak Mode"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gpt4only", action="store_true", help="Enable GPT4 Only Mode"
    )
    parser.add_argument(
        "--use-memory",
        "-m",
    ...
    if args.memory_type:
        ...
        if not chosen in supported_memory:
            ...
            logger.typewriter_log(
                f"Defaulting to: ", Fore.YELLOW, cfg.memory_backend
            )
        else:
